...
export const fetchPharmacys = () => (dispatch) => {
...
export const fetchPharmacyByAddress = (lat, lng) => (dispatch) => {
  dispatch({ type: LOADING_DATA });
  axios
    .get(`/Pharmacy-location/${lat}/${lng}`)
    .then((res) => {
      dispatch({
        type: SET_PHARMACYS,
        payload: res.data,
...
export const fetchPharmacy = (restId) => (dispatch) => {
  dispatch({ type: LOADING_DATA });
  axios
    .get(`/Pharmacy/${restId}`)
    .then((res) => {
...
export const changeBookingStatus = (orderId, body) => (dispatch) => {
  axios
    .post(`/booking-status/${orderId}`, body)
    .then((res) => {
      dispatch({
        type: EDIT_STATUS,
        payload: res.data.updatedOrder,
